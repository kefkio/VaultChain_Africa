...
import "../membership/IMembershipModule.sol";
...
contract LoanManager is AccessControl, ReentrancyGuard {
    ...
    constructor(address admin, address[] memory operators, address membershipContract) {
    ...
    function requestLoan(
        ...
        (, , , , , , , , , uint256 registeredAt, bool isActive) = membership.getMember(msg.sender);
        require(isActive, "Inactive member");
        require(block.timestamp >= registeredAt + 183 days, "Membership too recent");
    ...
    function disburseLoan(uint256 loanId) external onlyRole(OPERATOR_ROLE) {
        ...
        require(wallet == loan.borrower, "Wallet mismatch");

        loan.status = LoanStatus.Disbursed;

        if (loan.paymentType == PaymentType.Native) {
            (bool success, ) = wallet.call{value: loan.amount}("");
            require(success, "Native transfer failed");
        } else if (loan.paymentType == PaymentType.Token) {
            require(IERC20(loan.tokenAddress).transfer(wallet, loan.amount), "Token transfer failed");
        }
    ...
    function repayLoan(uint256 loanId, uint256 amount) external payable nonReentrant {
        ...
        if (loan.paymentType == PaymentType.Native) {
            ...
            require(IERC20(loan.tokenAddress).transferFrom(msg.sender, address(this), amount), "ERC20 transfer failed");
        }

        loan.amount -= amount;
        loan.status = loan.amount == 0 ? LoanStatus.FullyRepaid : LoanStatus.PartiallyRepaid;
